...
function jhSearch(doc, app, strQuery)
{
    display(doc, app.execQuery(strQuery));
}
<!--#else-->
...
var searchAnd = "%SearchAnd%".toUpperCase();
var searchOr = "%SearchOr%".toUpperCase();
var searchNot = "%SearchNot%".toUpperCase();
...
function getQueryResult(doc, newQuery, arr)
{
    ...
    if (newQuery.length > 0)
        res = "<div class=\"clsSearchDidYouMean\">%DidYouMean%: <a class=\"clsMouseOver\" href=\"" + doc.location.href.replace(doc.location.search, "?query=" + encodeURI(newQuery)) + "\">" + newQuery + "</a></div><br>" + res;
    return res;
}
